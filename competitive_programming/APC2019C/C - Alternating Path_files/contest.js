...
$(function() {
  ...
  if ($('.waiting-judge').length) {
    (function() {
      $('.waiting-judge').append('<img src="/public/img/icon/waiting.gif">');
      var timer = null;
      var interval = 3500;
      var sids = [];
      ...
      function reloadStatus() {
        ...
        var requestURL = '{0}?sids[]={1}'.format(submissionAPI, sids.join('&sids[]='));
        $.ajax(requestURL).done(function(data) {
          if (typeof data !== 'string') {
            $('.waiting-judge [data-toggle="tooltip"]').tooltip('hide');
            for (var id in data) {
              var html = $.parseHTML(data[id].Html);
              ...
              $('.submission-score[data-id={0}]'.format(id)).text(data[id].Score);
            }
            $('.waiting-judge [data-toggle="tooltip"]').tooltip();
          }
          timer = setTimeout(reloadStatus, interval);
        });
      }
      timer = setTimeout(reloadStatus, interval);
    })();
  ...
  // rejudge checkall
  if ($('#rejudge-btn').length) {
    $('#rejudge-checkall').change(function(){
      var state = $(this).prop("checked");
      $('input[name="submission_ids[]"]').prop("checked", state);
    });
    $('input[type="checkbox"]').click(function(){$('#rejudge-btn').show()});
  }
});
